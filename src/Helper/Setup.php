...
class Setup
{
        ...
        string $entityTypeTableName)
    {
        ...
        if ( ! $connection->isTableExists($entityTypeTableName)) {
            ...
            $entityTypeTable->addColumn('created_at', Table::TYPE_DATETIME, null, ['nullable' => false],
                'Creation Time');
    ...
    protected function addEntityAttributeType(SetupInterface $setup, string $entityTypeTableName, string $attributeType)
    {
        ...
        if ( ! $connection->isTableExists($entityAttributeTypeTableName)) {
            ...
            $entityAttributeTypeTable->addColumn('value_id', Table::TYPE_INTEGER, null, [
                'identity' => true,
                'nullable' => false,
                'primary'  => true,
            ], 'Value Id');
            $entityAttributeTypeTable->addColumn('entity_type_id', Table::TYPE_SMALLINT, null, [
                'unsigned' => true,
                'nullable' => false,
                'default'  => '0',
            ], 'Entity Type Id');
            $entityAttributeTypeTable->addColumn('entity_id', Table::TYPE_INTEGER, null, [
                'unsigned' => true,
                'nullable' => false,
                'default'  => '0',
            ], 'Entity Id');
            $entityAttributeTypeTable->addColumn('attribute_id', Table::TYPE_SMALLINT, null, [
                'unsigned' => true,
                'nullable' => false,
                'default'  => '0',
            ], 'Attribute Id');
            $entityAttributeTypeTable->addColumn('store_id', Table::TYPE_SMALLINT, null, [
                'unsigned' => true,
                'nullable' => false,
                'default'  => '0',
            ], 'Store ID');
            ...
            if ($attributeType == static::ATTRIBUTE_TYPE_DATETIME) {
                $entityAttributeTypeTable->addColumn('value', Table::TYPE_DATETIME, null,
                    ['nullable' => true, 'default' => '0000-00-00 00:00:00'], 'Value');
            } else if ($attributeType == static::ATTRIBUTE_TYPE_DECIMAL) {
                $entityAttributeTypeTable->addColumn('value', Table::TYPE_DECIMAL, [12, 4],
                    ['nullable' => true, 'default' => '0.0000'], 'Value');
            } else if ($attributeType == static::ATTRIBUTE_TYPE_INT) {
                $entityAttributeTypeTable->addColumn('value', Table::TYPE_INTEGER, 11,
                    ['nullable' => true, 'default' => 0], 'Value');
            } else if ($attributeType == static::ATTRIBUTE_TYPE_TEXT) {
                $entityAttributeTypeTable->addColumn('value', Table::TYPE_TEXT, null, ['nullable' => true], 'Value');
            } else if ($attributeType == static::ATTRIBUTE_TYPE_VARCHAR) {
                $entityAttributeTypeTable->addColumn('value', Table::TYPE_TEXT, 255, ['nullable' => true], 'Value');
            }

            $entityAttributeTypeTable->addIndex($setup->getIdxName($entityAttributeTypeTableName,
            ...
            $entityAttributeTypeTable->addIndex($setup->getIdxName($entityAttributeTypeTableName, ['entity_id']),
                ['entity_id']);
            $entityAttributeTypeTable->addIndex($setup->getIdxName($entityAttributeTypeTableName, ['attribute_id']),
                ['attribute_id']);
            $entityAttributeTypeTable->addIndex($setup->getIdxName($entityAttributeTypeTableName, ['store_id']),
                ['store_id']);

            $entityAttributeTypeTable->addForeignKey($setup->getFkName($entityAttributeTypeTableName, 'entity_id',
                $entityTypeTableName, 'entity_id'), 'entity_id', $entityTypeTableName, 'entity_id',
                Table::ACTION_CASCADE);
            $entityAttributeTypeTable->addForeignKey($setup->getFkName($entityAttributeTypeTableName, 'attribute_id',
                $eavAttributeTableName, 'attribute_id'), 'attribute_id', $eavAttributeTableName, 'attribute_id',
                Table::ACTION_CASCADE);
            $entityAttributeTypeTable->addForeignKey($setup->getFkName($entityAttributeTypeTableName, 'store_id',
                $magentoStoreTableName, 'store_id'), 'store_id', $magentoStoreTableName, 'store_id',
                Table::ACTION_CASCADE);
    ...
    public function addProductAttributeToSetAndGroup(
        ...
        int $attributeSortOrder = null): bool
    {
                        ...
                        if (is_array($attributeData)) {
                            ...
                            if ($attributeId) {
                                $setup->addAttributeToGroup($entityTypeId, $attributeSetId, $attributeGroupId,
                                    $attributeId, $attributeSortOrder);
